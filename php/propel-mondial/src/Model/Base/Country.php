...
abstract class Country implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Model\\Map\\CountryTableMap';
    ...
    public function setProvince($v)
    {
        ...
        if ($this->province !== $v) {
            $this->province = $v;
            $this->modifiedColumns[CountryTableMap::COL_PROVINCE] = true;
        }
    ...
    public function setArea($v)
    {
        ...
        if ($this->area !== $v) {
            $this->area = $v;
            $this->modifiedColumns[CountryTableMap::COL_AREA] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : CountryTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : CountryTableMap::translateFieldName('Capital', TableMap::TYPE_PHPNAME, $indexType)];
            $this->capital = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : CountryTableMap::translateFieldName('Province', TableMap::TYPE_PHPNAME, $indexType)];
            $this->province = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : CountryTableMap::translateFieldName('Area', TableMap::TYPE_PHPNAME, $indexType)];
            $this->area = (null !== $col) ? (double) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : CountryTableMap::translateFieldName('Population', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(CountryTableMap::COL_PROVINCE)) {
            $modifiedColumns[':p' . $index++]  = 'Province';
        }
        if ($this->isColumnModified(CountryTableMap::COL_AREA)) {
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setCapital($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setProvince($arr[$keys[3]]);
        }
        if (array_key_exists($keys[4], $arr)) {
            $this->setArea($arr[$keys[4]]);
        }
        if (array_key_exists($keys[5], $arr)) {
            $this->setPopulation($arr[$keys[5]]);
        }
    }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Model\Country The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(CountryTableMap::COL_PROVINCE)) {
            $criteria->add(CountryTableMap::COL_PROVINCE, $this->province);
        }
        if ($this->isColumnModified(CountryTableMap::COL_AREA)) {
            $criteria->add(CountryTableMap::COL_AREA, $this->area);
        }
        if ($this->isColumnModified(CountryTableMap::COL_POPULATION)) {
