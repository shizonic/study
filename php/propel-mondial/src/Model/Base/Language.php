...
abstract class Language implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Model\\Map\\LanguageTableMap';
    ...
    /**
     * Initializes internal state of Model\Base\Language object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\Model\Language The current object (for fluent API support)
     */
    public function setCountry($v)
    {
        ...
        if ($this->country !== $v) {
            $this->country = $v;
            $this->modifiedColumns[LanguageTableMap::COL_COUNTRY] = true;
        }
    ...
    public function setPercentage($v)
    {
        ...
        if ($this->percentage !== $v) {
            $this->percentage = $v;
            $this->modifiedColumns[LanguageTableMap::COL_PERCENTAGE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : LanguageTableMap::translateFieldName('Country', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : LanguageTableMap::translateFieldName('Percentage', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = LanguageTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Model\\Language'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(LanguageTableMap::COL_COUNTRY)) {
            $modifiedColumns[':p' . $index++]  = 'Country';
        }
        if ($this->isColumnModified(LanguageTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'Name';
        }
        if ($this->isColumnModified(LanguageTableMap::COL_PERCENTAGE)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Model\Language The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(LanguageTableMap::COL_COUNTRY)) {
            $criteria->add(LanguageTableMap::COL_COUNTRY, $this->country);
        }
        if ($this->isColumnModified(LanguageTableMap::COL_NAME)) {
            $criteria->add(LanguageTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(LanguageTableMap::COL_PERCENTAGE)) {
            $criteria->add(LanguageTableMap::COL_PERCENTAGE, $this->percentage);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Model\Language (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Model\Language Clone of current object.
     * @throws PropelException
     */
