...
public class LinkImpl implements Link{
	...
	public LinkImpl( City from, City to, Double weight ){
		init(from,to,weight);
	}
	public LinkImpl( City from, City to ){
		init(from,to,null);
	}

	private void init( City from, City to, Double weight){
		this.from=from;
		this.to=to;
		this.weight=(weight!=null)?weight:getDirectDistance();
	}
	...
	public boolean isIncludedIn(City[] cities) {
		...
		for( int i=0;i<cities.length;i++){
			City c = cities[i];
			if( !foundFrom && (c.id == from().id)){
				foundFrom = true;
			}
			if( !foundTo && (c.id == to().id)){
				foundTo = true;
			}
