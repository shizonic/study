...
public class RouteFeeder {
	 ...
	 * @throws IOException
	 */
	public int loadLinks( String filename ) throws IOException{
		return loadLinks(new BufferedReader( new InputStreamReader (
				getClass().getResourceAsStream(filename) ) ));
	}
	
	private int loadLinks(BufferedReader br) throws IOException {
		int linkcount=0;
		int linecount = 0;
		String line;
		while ((line = br.readLine()) != null) {
			...
			if( recs.length==2){
				...
				System.out.println(Integer.toString(linecount) + ": " +line);
			}
			linecount++;
		}
		return linkcount;
