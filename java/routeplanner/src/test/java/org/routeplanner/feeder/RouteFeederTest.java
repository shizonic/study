...
public class RouteFeederTest {
	...
	public void findCitiesAndRoutes(){
		...
		City cities[] = routeMgr.findCitiesBetween(lyon, berlin);
 		Assert.assertEquals(452, cities.length);		
		Link routes[] = routeMgr.findAllLinks(cities);
		Assert.assertEquals(126, routes.length);
	}
	@Test
	public void findShortestPath() throws FileNotFoundException, IOException {
		...
		double dist = 0;
		String [] itCities = {"Lyon", "Genève", "Lausanne","Fribourg","Bern","Biel", "Delémont", "Basel", "Freiburg", "Karlsruhe", "Mannheim", "Frankfurt","Erfurt", "Leipzig","Berlin"};
		...
		Assert.assertEquals(1062.29, dist, 0.1);
	}
}
